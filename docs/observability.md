...
| Camera microphone | Tonal and broadband sound in the actual room | Audio sample rate | Acoustic comparison only when position, gain, room state, speed, and load are matched. It cannot localize motor versus controller without electrical correlation or an A/B change. |
