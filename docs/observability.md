...
| V2 TMP1075 | Chamber air plus residual local-board heating at the U12 edge location | 1 Hz | PCB-01 V2 chamber-temperature proxy. Firmware reports optional integer milli-C telemetry. It is not the U2 hotspot measurement and has no drive or safety authority. |
...
| V1 motor NTC | Stationary GL100/MC-100 motor-mount temperature | Slow | V1 J4 and the purchased ring-lug NTC exist, but firmware acquisition remains TODO. V2 deletes this interface. Until implemented on V1, read the divider with an external meter/logger or use an independent attached temperature logger for release thermal tests. |
