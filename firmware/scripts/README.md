...
The runner uses the ESP32-C6 native USB Serial/JTAG port. Power the board normally, hold BOOT, tap
RESET, release BOOT, then run the flash step. USB VBUS does not power the board. Set
`STILLAIR_PORT=/dev/cu...` to override detection. There is no FTDI/UART transport, VID/PID-specific
adapter logic, or RTS/DTR automation.
