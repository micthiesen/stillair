...
export const componentDefinitions: Record<
  Ref,
  { value: string; mpn?: string; symbol: string; pads: string[]; datasheet?: string }
> = {
  ...
  J1: {
    ...
    pads: ["1", "2", "3", "4"],
    datasheet: "https://www.jst-mfg.com/product/index.php?series=199",
  },
  J2: {
    value: "S8B-PH-K-S",
    mpn: "S8B-PH-K-S",
  ...
  U1: {
    value: "SC18IS606PWJ",
    mpn: "SC18IS606PWJ",
    symbol: "Interface_Expansion:SC18IS606PW",
    pads: Array.from({ length: 16 }, (_, index) => String(index + 1)),
    datasheet:
      "https://www.nxp.com/products/interfaces/ic-spi-i3c-interface-devices/bridges/ic-bus-to-spi-bridge%3ASC18IS606",
  },
  U2: {
    value: "PCA9536DR",
    mpn: "PCA9536DR",
    symbol: "Interface_Expansion:PCA9536D",
    pads: Array.from({ length: 8 }, (_, index) => String(index + 1)),
    datasheet: "https://www.ti.com/product/PCA9536",
  },
};

const stableComponentId = (ref: string) => `pcb03.component.${ref.toLowerCase()}`;
const stableNetId = (name: string) =>
  `pcb03.net.${(netAliases as Record<string, string>)[name]?.toLowerCase() ?? name.toLowerCase()}`;
