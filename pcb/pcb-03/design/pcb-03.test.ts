...
describe("PCB-03 authoritative fixture", () => {
  test("pins the validated tscircuit toolchain", () => {
    ...
    expect(String(designManifest.versions.tscircuit)).toBe(
      packageJson.dependencies.tscircuit,
    );
    expect(String(designManifest.versions.circuit_json_to_kicad)).toBe(
      packageJson.dependencies["circuit-json-to-kicad"],
    );
  });
  ...
  test("manual placement and stable manifest agree exactly", () => {
    ...
    for (const edit of manualEdits.pcb_placements) {
      ...
      const manifestComponent = designManifest.components.find(
        ({ ref }) => ref === edit.selector,
      );
  ...
  test("Circuit JSON has the exact board and twelve placed references", () => {
    ...
    const sources = new Map(
      byType("source_component").map((item) => [item.source_component_id, item.name]),
    );
    ...
    for (const pcb of pcbs) {
      ...
      if (pcb.display_offset_x !== undefined || pcb.display_offset_y !== undefined) {
        expect({ x: pcb.display_offset_x, y: pcb.display_offset_y }).toEqual({
          x: expected.x,
          y: expected.y,
        });
  ...
  test("manifest component identities, values, symbols, pads, and holes match Circuit JSON", () => {
    const sources = new Map(
      byType("source_component").map((item) => [item.name, item]),
    );
    const pcbs = new Map(
      byType("pcb_component").map((item) => [item.source_component_id, item]),
    );
    const sourceById = new Map(
      byType("source_component").map((item) => [item.source_component_id, item]),
    );
    const portsByComponent = Object.groupBy(
      byType("source_port"),
      ({ source_component_id }) => source_component_id,
    );
    const schematicIds = new Set(
      byType("schematic_component").map(
        ({ source_component_id }) => source_component_id,
      ),
    );
    ...
    for (const manifestComponent of designManifest.components) {
      ...
      if (!source) continue;
      expect(source.manufacturer_part_number).toBe(
        manifestComponent.fields.manufacturer_part_number ?? manifestComponent.value,
      );
      if (source.ftype === "simple_resistor")
        expect(source.resistance).toBe(
          Number.parseFloat(manifestComponent.value) * 1_000,
        );
      ...
      expect(
        (portsByComponent[source.source_component_id] ?? [])
          .map(({ pin_number }) => String(pin_number))
          .sort(),
      ).toEqual([...expectedPads].sort());
      expect(manifestComponent.symbol).toBe(
        componentDefinitions[manifestComponent.ref].symbol,
      );
      expect(schematicIds.has(source.source_component_id)).toBe(
        !manifestComponent.ref.startsWith("H"),
      );
      expect(
        pcbs.get(source.source_component_id)?.metadata.kicad_footprint.footprintName,
      ).toBe(manifestComponent.footprint.kicad);
    }
    ...
    for (const expected of designManifest.board.holes) {
      ...
      const hole = holes.find(
        ({ pcb_component_id }) => pcb_component_id === pcb?.pcb_component_id,
      );
  ...
  test("Circuit JSON connectivity matches every specified pin and net", () => {
    ...
    const ports = new Map(
      byType("source_port").map((item) => [item.source_port_id, item]),
    );
    const sourceNets = new Map(
      byType("source_net").map((item) => [item.source_net_id, item.name]),
    );
    ...
    for (const trace of byType("source_trace")) {
      for (const netId of trace.connected_source_net_ids ?? []) {
        ...
        for (const portId of trace.connected_source_port_ids ?? []) {
          ...
          if (!port) continue;
          members.add(
            `${sourceComponents.get(port.source_component_id)}.${port.pin_number}`,
          );
        }
  ...
  test("build contains no error elements and handoff augmentation is explicit", () => {
    ...
    const warningClasses = new Set(
      circuit.filter(({ type }) => type.endsWith("warning")).map(({ type }) => type),
    );
    expect(
      [...warningClasses].filter((type) => !reviewedWarningClasses.has(type)),
    ).toEqual([]);
    expect(augmentation.schema_version).toBe(1);
    expect(augmentation.board_id).toBe(designManifest.board.stable_id);
    expect(new Set(augmentation.operations.map(({ id }) => id)).size).toBe(
      augmentation.operations.length,
    );
    expect(augmentation.operations.every(({ owner }) => owner === "kicad")).toBe(true);
    expect(
      augmentation.operations.find(({ kind }) => kind === "net_alias"),
    ).toMatchObject({
      target: { net_stable_id: "pcb03.net.3v3" },
      params: { source_name: "V3V3", kicad_name: "3V3" },
    });
    const footprintOverrides = augmentation.operations.filter(
      ({ kind }) => kind === "footprint_override",
    );
    expect(footprintOverrides).toHaveLength(12);
    for (const component of designManifest.components) {
      expect(
        footprintOverrides.find(
          ({ target }) => target.component_stable_id === component.stable_id,
        ),
      ).toMatchObject({
